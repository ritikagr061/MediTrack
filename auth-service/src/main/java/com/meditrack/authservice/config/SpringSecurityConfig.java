...
//import com.meditrack.authservice.jwt.JwtFilter;
...
//    private JwtFilter jwtFilter;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        ...
        http.authorizeHttpRequests(customizer->customizer
                ...
                .requestMatchers("/auth/login","/auth/register-user", "/auth/register", "/auth/hospitals/**")
                .permitAll()
                .anyRequest().authenticated())
        ;
        //to allow clients like postman to interact
        http.httpBasic(Customizer.withDefaults());
        // by default spring security assumes you are building web app, so provides basic Auth
        // and also creates session(JSESSIONID), stores the user’s authentication (SecurityContext) in that session,
        // Every future request reads that session to re-authenticate automatically, we are disabling that by making it stateless.
